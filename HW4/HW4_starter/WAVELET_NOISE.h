...
static void downsampleXNeumann(float* to, const float* from, int sx, int sy) {
  for (int iy = 0; iy < sy; iy++) {
      const int i = iy * sx;
      downsampleNeumann(&from[i], &to[i], sx, 1);
    }
}
static void downsampleYNeumann(float* to, const float* from, int sx, int sy) {
...
static void upsampleXNeumann(float* to, const float* from, int sx, int sy) {
  for (int iy = 0; iy < sy; iy++) {
      const int i = iy * sx;
      upsampleNeumann(&from[i], &to[i], sx, 1);
    }
}
static void upsampleYNeumann(float* to, const float* from, int sx, int sy) {
  for (int ix = 0; ix < sx; ix++) {
      const int i = ix;
...
static void generateTile(float* const noiseTileData, std::string filename) {
  ...
  const int n2 = n*n;
  std::cout <<"Generating new 2d noise tile size="<<n<<"^2 \n";
  MTRand twister;

  float *temp13 = new float[n2];
  float *temp23 = new float[n2];
  float *noise3 = new float[n2];
  ...
  delete[] temp13;
  delete[] temp23;
  std::cout <<"Generating new 2d noise done\n";
}
